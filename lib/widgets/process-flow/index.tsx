/* ROUTES-FROM-COLUMN:BEGIN — generated by scripts/generate_widget_routing_docs.py; do not edit by hand.
   ...
   32 concept(s):
   - [biol11 ch3] Alternation of Generations and Life Cycle Patterns
     Chunks give the cycle as an explicit arrow chain: 'gametes (n) -fusion-> zygote (2n) -meiosis-> zoospores (n) -> gametophyte', with the three patterns
   - [biol11 ch11] Calvin Cycle: Carboxylation, Reduction and Regeneration
     Explicit figure spec: 'Circular Calvin cycle. Stage 1 Carboxylation ... Stage 2 Reduction ... Stage 3 Regeneration, closing the loop.' A closed ring o
   - [biol11 ch12] Glycolysis: Steps, Enzymes and Net Yield
     Explicit 'Figure - to be illustrated: Vertical glycolysis flowchart. Glucose -hexokinase,-ATP-> G6P -> F6P -> F1,6-bisP -aldolase-> G3P/DHAP ... -> Py
   - [biol11 ch12] Krebs Cycle: Reactions and Products
     Explicit figure: 'Circular Krebs cycle wheel... Acetyl CoA enters at top, condenses with OAA -> Citrate ... clockwise ... Malate -> OAA' - a closed ri
   - [biol11 ch13] Differentiation, Dedifferentiation and Redifferentiation
     Chunk: 'Figure A three-step cyclic flow - Meristem cell -(Differentiation)-> Mature cell -(Dedifferentiation)-> regains division -(Redifferentiation)-
   - [biol11 ch15] Blood Coagulation and Clotting Factors
     Chunks give the cascade explicitly: spasm -> prothrombinase -> 'Prothrombin -prothrombinase/Ca2+-> Thrombin' -> 'Fibrinogen -thrombin-> Fibrin', with 
   - [biol11 ch16] Hormonal Regulation of Kidney Function: RAAS, ADH and ANF
     Figure: 'Two-arm regulation schematic. LEFT LOW BP: JGA -> renin -> angiotensinogen -> A-I -> A-II -> aldosterone -> BP restored; RIGHT: atria -> ANF,
   - [biol11 ch17] Sliding Filament Theory of Muscle Contraction
     Chunk renders 'The cross-bridge cycle (one myosin head) 1. ATP cocks head 2. binds actin site 3. power stroke 4. detach, new ATP binds -> cycle' - an 
   - [biol11 ch18] Reflex Action and the Reflex Arc
     Explicit five-component ordered arc plus 'Figure. A reflex arc ... receptor -> afferent neuron -> interneuron -> efferent neuron -> effector'. Linear 
   - [biol12 ch2] Cleavage, Blastocyst Formation and Implantation
     'Diagram. Linear sequence from zygote in the oviduct -> 2-cell -> 4-cell -> morula -> blastocyst (labelling trophoblast and inner cell mass) -> blasto
   - [biol12 ch2] Gametogenesis: Spermatogenesis and Oogenesis
     Explicit chains: 'spermatogonium -> primary -> secondary spermatocyte -> spermatid -> spermatozoon -> spermiation' and the oogenesis line; chunk even 
   - [biol12 ch2] Parturition and Its Hormonal Control
     'Diagram. Positive-feedback loop for parturition - signals -> mild uterine contractions -> oxytocin -> stronger contractions -> more oxytocin, drawn a
   - [biol12 ch3] Infertility and Assisted Reproductive Technologies (IVF, ZIFT, GIFT, ICSI)
     Fig 3.4 is spelled out as a linear strip: '(1) ovum+sperm collected -> (2) fused in culture dish -> (3) zygote -> (4) early embryo (<=8 cells) -> (5) 
   - [biol12 ch5] Transcription in Prokaryotes and Eukaryotes
     'three stages, each with a clear logic: 1. Initiation ... 2. Elongation ... 3. [Termination]' plus the hnRNA pipeline 'Step 1 cap, Step 2 splicing, St
   - [biol12 ch5] Translation: Ribosomes, tRNA and Protein Synthesis
     'The three stages of translation. Initiation ... Elongation: 1. A charged tRNA enters the A-site. 2. The ribozyme catalyses a peptide bond. 3. [transl
   - [biol12 ch7] Malaria and the Life Cycle of Plasmodium
     Explicit figure spec: 'Circular life-cycle of Plasmodium spanning two hosts... arrows clockwise: bite -> sporozoites -> liver -> RBC -> gametocytes ->
   - [biol12 ch8] Microbes in Household Products (Curd, Dough, Cheese, Toddy)
     Chunk: 'Figure - to be drawn. A simple 4-stage horizontal flow strip: (1) warm milk; (2) starter added; (3) arrow lactose to lactic acid, pH falls; (4
   - [biol12 ch8] Sewage Treatment: Primary, Secondary and BOD
     Ordered pathway in the chunks: filtration/sedimentation -> primary sludge + effluent -> aeration tank/flocs -> BOD drops -> activated sludge -> anaero
   - [biol12 ch9] PCR: Principle, Steps and Applications
     Chunk figure is a cycle, not a machine: '(A) denaturation ~94C, (B) annealing ~40-60C, (C) extension ~72C by Taq; arrow looping back x30 cycles'. No t
   - [biol12 ch9] Principles of Biotechnology: Genetic Engineering and Bioprocess Engineering
     Chunk states: 'Diagram - Three-step logical flow - (1) Identify DNA with desirable gene, arrow to (2) Introduce DNA into host, arrow to (3) Maintain a
   - [biol12 ch9] Steps in Recombinant DNA Technology (Gene Cloning Sequence)
     Chunk: 'Diagram - Linear rDNA workflow flowchart - Isolation -> Cutting -> Amplification by PCR -> Ligation -> Insertion into host -> Selection -> Cul
   - [biol12 ch10] Gene Therapy and ADA Deficiency Treatment
     Chunk: 'Visualise this - Cyclic flowchart. (1) lymphocytes taken from blood (2) grown in culture (3) ADA cDNA into retrovirus (4) retrovirus delivers 
   - [biol12 ch10] Recombinant Human Insulin (Humulin) Production
     Five numbered steps: synthesise A and B genes -> insert into E. coli plasmids -> express separately -> extract and purify -> assemble by disulphide bo
   - [biol12 ch12] Carbon Cycle
     Chunk carries a literal 'Diagram to render': box 'CO2 in atmosphere (reservoir)' with arrows photosynthesis down, respiration/decomposition/combustion
   - [biol12 ch12] Decomposition and Its Steps
     Chunk: 'Diagram to render - a left-to-right pipeline of five linked boxes on the detritus stream', with the fixed order fragmentation -> leaching -> c
   - [biol12 ch12] Ecological Succession: Primary, Secondary, Hydrarch and Xerarch
     Explicit ordered sequence: 'bare rock -> crustose lichens -> foliose lichens -> mosses -> herbs -> shrubs -> forest (climax)'; hydrarch parallel chain
   - [biol12 ch12] Energy Flow and the Ten Per Cent Law
     Chunks trace an ordered chain Sun->producers->herbivores->consumers with ~10% passed per arrow and ~90% lost as heat; 'count the arrows... that many f
   - [biol12 ch12] Phosphorus Cycle
     Same 'Diagram to render' chunk: 'Rock phosphate (reservoir)' -> weathering -> soil solution -> producers -> decomposers -> back to soil, dashed arrow 
   - [chem11 ch4] Ionic Bond Formation, Lattice Enthalpy and Born-Haber Cycle
     Chunk: 'Figure A closed Born-Haber cycle for an alkali halide MX' - boxes M(s)+1/2X2, M(g), M+(g), X-(g), MX(s) joined by sublimation/ionisation/elect
   - [math11 ch8] Sequences: General Term and Recursive Definitions
     Chunk 239f95eb carries Figure 8.1 in full: three labelled boxes joined by arrows, 'input n=1,2,3' -> 'rule an=2n-1' -> 'output 1,3,5,7', with 'positio
   - [math12 ch2] Sum and Difference Formulae for tan Inverse
     Figure 2.8 verbatim: 'A decision tree for tan-1 x + tan-1 y: compute xy. If xy<1 use directly. If xy>1: add pi when x,y>0, subtract pi when x,y<0.' Or
   - [phys11 ch11] Zeroth Law and Thermal Equilibrium
