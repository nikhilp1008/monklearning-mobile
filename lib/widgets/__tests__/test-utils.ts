...
/**
 * Compare a freshly rendered tree against a frozen golden — or, under
 * `REGENERATE_GOLDEN=1`, overwrite the golden with it.
 *
 * The repo already had this policy written down twice
 * (`__generate__/golden.gen.ts`, `scripts/regenerate-golden.mjs`: "Regenerate
 * deliberately, never automatically. A golden file that updates itself is a
 * test that cannot fail.") and a MECHANISM for exactly one of the fourteen
 * golden trees. The other thirteen — xy_plot's nine and field_lines' four —
 * could only be regenerated by hand-editing JSON, which is how a golden stops
 * being regenerated at all and starts being deleted instead.
 *
 * The guard is the same one `golden.gen.ts` uses, so `npm run
 * regenerate-golden` is still the only route in and a plain `npm test` can
 * never rewrite a golden it was supposed to be checked by.
 */
export function assertGolden(
  ...
  if (process.env.REGENERATE_GOLDEN === '1') {
    writeFileSync(resolve(dir, file), JSON.stringify(tree, null, 1) + '\n');
    return;
  }
