...
**New structure:** one row — greeting on the left, profile avatar on the right, closed by a `1px solid rgba(28,25,20,0.08)` bottom rule. Greeting is `27px / weight 500 / -0.025em`, single line, `ellipsis` on overflow. Avatar 38px.
...
An intermediate version darkened the page to `#F4F0E5`; that was reverted. The background is `#FCFAF4`, unchanged from the original.
